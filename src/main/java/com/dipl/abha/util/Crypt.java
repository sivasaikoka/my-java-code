...
public class Crypt {
	...
	public static PublicKey getPublicKey(String algo, byte[] jceBytes) throws Exception {
		...
		return KeyFactory.getInstance(algo, BouncyCastleProvider.PROVIDER_NAME).generatePublic(x509EncodedKeySpec);
	}
	...
	public String generateSignatures(String payload, String signatureAlgorithm, PrivateKey privateKey) {
		try {
	...
	public String generateSignature(String req, PrivateKey privateKey) {
		return generateSignatures(req, SIGNATURE_ALGO, privateKey);
	}

	public String generateAuthorizationParams(String payload, String subscriberId, String pubKeyId) throws Exception {
		...
		map.put("signature", generateSignature(generateBlakeHash(getSigningString(created_at, expires_at, payload)),
	...
	public String getSigningString(long created_at, long expires_at, String payload) {
		return "(created): " + created_at + " (expires): " + expires_at + " digest: BLAKE-512=" + hash(payload);
