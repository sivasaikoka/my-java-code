...
@Service
public class HipLinkingService {
	...
	public ResponseEntity<?> restCallApi(Object payload, String url, String apiType, HttpServletRequest request)
		...
		try {
			...
			headers.set("X-HIP-ID", request.getHeader("X-HIP-ID"));
			headers.set("TIMESTAMP", request.getHeader("TIMESTAMP"));
			headers.set("REQUEST-ID", request.getHeader("REQUEST-ID"));

			if ("LINK-CARE-CONTEXT".equals(apiType)) {
				headers.set("X-LINK-TOKEN", request.getHeader("X-LINK-TOKEN"));
			}
